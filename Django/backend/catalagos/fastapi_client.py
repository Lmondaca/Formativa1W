...
def create_juego(data):
    ...
    try:
        r = requests.post(f"{BASE_URL}/juegos/", json=data_copy)
        r.raise_for_status()
        return r.json()
...
def actualizar_juego(juego_id, data):
    ...
    r = requests.put(f"{BASE_URL}/juegos/{juego_id}", json=data_copy)
    r.raise_for_status()
    return r.json()

def obtener_juego(juego_id):
    r = requests.get(f"{BASE_URL}/juegos/{juego_id}")
    r.raise_for_status()
    return r.json()

def eliminar_juego(juego_id):
    r = requests.delete(f"{BASE_URL}/juegos/{juego_id}")
    r.raise_for_status()
    return r.json()